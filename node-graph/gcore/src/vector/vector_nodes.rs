...
async fn sample_points(_: impl Ctx, vector_data: VectorDataTable, spacing: f64, start_offset: f64, stop_offset: f64, adaptive_spacing: bool, subpath_segment_lengths: Vec<f64>) -> VectorDataTable {
	...
	for mut vector_data_instance in vector_data.instance_iter() {
		...
		let mut bezier = vector_data_instance.instance.segment_bezier_iter().enumerate().peekable();
		while let Some((index, (segment_id, _, start_point_index, mut last_end))) = bezier.next() {
			...
			for c in 0..=max_c {
				...
				let Some(segment) = vector_data_instance.instance.segment_from_id(current_segment_id) else {
					continue;
				};
				let segment = segment.apply_transformation(|point| vector_data_instance.transform.transform_point2(point));
				...
				result.point_domain.push(point_id, vector_data_instance.transform.inverse().transform_point2(point));
